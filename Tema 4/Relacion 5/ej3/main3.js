...
function solicitarNombre(app, cookieName) {
  ...
  // valido si existe nombre o se deja vacio
...
function mostrarSaludo(nombre, app, cookieName) {
  ...
  document.getElementById("btn-cambios").addEventListener("click", () => {
    ...
    crearCookie("colorFondo", colorFondo); //le quito el tiempo de expiracion para que al cerrar el navegador se borren estas cookies de preferencias
    crearCookie("colorTexto", colorTexto); //esto pasa xq el navegador ignora la fecha de expiracion de estas cookies al dar un valor null en su calculo del setTime
    crearCookie("tamaño", tamaño); // sin embargo, otro navegador puede que con no crre la cookie en esta situacion
  });
}
...
function crearCookie(cname, cvalue, minutes) {
  //controlo el caso de que no se le pase minutos en la llamada para el borrado del ejercicio 3
  let expires = "";
  if (minutes) {
    let d = new Date();
